...
jetmemo's reliability was measured on Opus-class models. The exposure here is larger than in a single-context skill: Phase 2 fans out to as many as seven parallel readers and **this context never reads the briefs or the record itself** — it sees only what those agents report back. A weaker reader's miss surfaces nowhere downstream, and the two blocking gates that would otherwise catch it (Step 2.5's essential-documents gate, Step 2.6's preservation trace) are the model's own assessment of its own reading. Confirm the runtime model before any of that starts.

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/check_model.py"
```

Stdlib only — system `python3`, ahead of the refs setup and the update check. It prints one line, `MODEL_GATE: <status> model=<id> tier=<family> source=<origin>`, and exits 0 (`ok`), 2 (`warn`), or 3 (`unknown`). **A nonzero exit is the gate firing, not a broken script.**
...
- **`unknown`** — the model could not be detected (no transcript yet, no `$CLAUDE_CODE_SESSION_ID`). Identify yourself from your own system prompt and re-run once with that id — substitute your actual model, do not copy the example:
  ```bash
  python3 "${CLAUDE_SKILL_DIR}/scripts/check_model.py" --model claude-opus-5
  ```
  If it still reports `unknown`, treat it as `warn`.
- **`warn`** — a Sonnet- or Haiku-class model, or any model name the gate does not recognize. **Stop. Scan no PDFs, split nothing, spawn no agent, until the user answers.**
...
```
> **Reduced-reliability model.** This memo was generated on [model id], outside the Opus-class set jetmemo's reliability testing is based on. Its record citations, preservation findings, and brief coverage carry a higher miss rate than the same run on an Opus-class model.
```

Do not repeat the warning in every issue or in every agent's output. Note that Step 6.5's jetredline audit runs its own gate and reports the model under `### Coverage` in its results — that is an independent cross-check, not a duplicate to suppress.

**Where `AskUserQuestion` is unavailable — headless `claude -p`, or any caller that cannot answer — never block.** State the warning once, add the memo line above, and run the full pipeline. A caller that cannot answer a question must still get its memo.
...
4. **Extract citation list:** Run the citation checker on all `.txt` files to build a structured citation list. This determines which conditional agents to launch.
   ...
   **Splice-suspect entries.** A neutral-cite entry with `splice_suspect: true` may have captured page furniture (a page-number footer) as its opinion number — the matched text spans a line break and a bare number immediately follows it in the brief. `splice_repair_candidate` holds the rejoined cite (e.g. the phantom `2025 ND 13` with candidate `2025 ND 127`). Agent D must verify the candidate before reporting the original, and must never report any cite as nonexistent or as an unrelated case without first attempting this repair.
...
> - `splice_suspect` / `splice_repair_candidate`: the cite may have captured a page-number footer as its opinion number. Verify `splice_repair_candidate` first; report the original number only if the candidate also fails. **Never report a cite as nonexistent or as an unrelated case without attempting this repair** — also apply it when an unflagged neutral cite fails to resolve or its `antecedent_name` clearly names a different case than the resolved caption.
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/provenance.py" --file {memo_file} --subagents
python3 "${CLAUDE_SKILL_DIR}/scripts/memo_to_docx.py" {memo_file}
```
...
`--subagents` adds a table of the delegated analysis agents below that line — the model each ran on and what it consumed:

| Agent | Model | Tokens | Tools | Time |
|---|---|---|---|---|
| Issue 1 — standard of review | claude-opus-5[1m] | 88.4k | 21 | 3m 12s |
| Record chronology | claude-opus-5[1m] | 64.9k | 14 | 2m 05s |

- Rows are labelled from each agent's `description`, so **give every delegated agent a short, specific description** ("Issue 2 preservation analysis", not "analysis"). Only numbers, the model ID, and that description cross into the report; no prompt or result text does.
- **Tokens is a single total per agent.** Background subagents — the default — report completion through a `<task-notification>` carrying one `subagent_tokens` figure with no input/output/cache breakdown, so a wider schema cannot be filled. Synchronous agents still emit the older self-contained record, which the script also reads; those rows show `—` for Model, which that record omits.
- The table is **omitted** whenever nothing was delegated or the transcript can't be read — the transcript is an undocumented internal format, so a missing table is expected behavior, never an error to chase. Do **not** hand-write it from your own recollection of the agents.
- **Model, version, and date are all sourced deterministically by the script** — version from this skill's `SKILL.md` frontmatter (then a `VERSION` file), date from the system clock, and the model from this session's transcript, the same place the agent rows above come from. Do not hand-type any of them.
- **Do not pass `--model`.** Your own recollection of which model you are is exactly the self-report a validation record must not rest on. If the script exits 2 because no transcript is readable, then and only then re-run it with `--model "<friendly name> (<exact id>)"` using the id Step 0.0 resolved.
