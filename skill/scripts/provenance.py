...
check_model.py — the same source the delegated-agent rows below the sentence
...
#
# Tier 1 (authoritative): in jetredline every delegated pass is launched with a
...
# "You are running Pass 4 (fact-check) for an audit...", and a
...
# This skill delegates un-numbered analysis agents rather than numbered passes
# with instruction files, so tiers 1 and 2 never fire and labels come from the
# launch record's `description` (tier 3). Populating these tables and adding
# references/pass-instructions/passN-*.md files would switch it to tier 1.
PASS_LABELS = {}
...
def detect_model(session_id=None, transcript=None):
    ...
    jetmemo and jetredline but not in every skill that vendors provenance, so
...
def find_skill_dir(script_path: Path) -> Path:
    ...
    Works whether the script lives in <skill>/scripts/ (as here)
